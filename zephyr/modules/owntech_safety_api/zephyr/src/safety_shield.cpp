...
/* *
 * Define the number of channels that need to be monitored for safety purpose.
*/
...
/**
 * @brief Initializes the threshold min and max for all the channels with th default value
...
void safety_init_shield(bool watch_all)
{
    ...
    {
        uint8_t rc = safety_retrieve_threshold_in_nvs(dt_threshold_props[i].channel);
        if(rc != 0)
        {
            printk("%s value not found in static storage. Default value will be used \n", dt_threshold_props[i].name);
            safety_set_channel_threshold_max(&( dt_threshold_props[i].channel ),
                                            (float32_t*)(&( dt_threshold_props[i].threshold_max )), 1);
            safety_set_channel_threshold_min(&( dt_threshold_props[i].channel ),
        ...
        if(watch_all) safety_set_channel_watch(&( dt_threshold_props[i].channel ), 1);
    }
}
